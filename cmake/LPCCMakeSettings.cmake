...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# By default the builtin ``GNUInstallDirs`` module is included to provide
...
#-----------------------------------------------------------------------
# "Header guard"
if(__LPCCMAKESETTINGS_INCLUDED)
  return()
else()
...
foreach(_dir BINDIR LIBDIR INCLUDEDIR DATAROOTDIR)
  if(IS_ABSOLUTE "${CMAKE_INSTALL_${_dir}}")
    message(FATAL_ERROR "[fatal] LPCCMakeSettings: Absolute path for CMAKE_INSTALL_${_dir} not allowed")
  endif()
endforeach()
...
set(CMAKE_INSTALL_PLUGINDIR "${CMAKE_INSTALL_LIBDIR}/${PROJECT_NAME}/plugins")
set(CMAKE_INSTALL_FULL_PLUGINDIR "${CMAKE_INSTALL_FULL_LIBDIR}/${PROJECT_NAME}/plugins")
...
if(${PROJECT_NAME_UC}_CXX_STANDARD EQUAL 14)
  if(NOT ${PROJECT_NAME_UC}_CXX14_COMPILE_FEATURES)
    message(FATAL_ERROR "[fatal] LPCCMakeSettings: C++14 requested, but no support for any C++14 features for compiler:\n"
